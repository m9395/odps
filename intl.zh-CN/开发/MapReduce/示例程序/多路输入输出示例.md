...
3.  使用Tunnel导入数据。
    ...
    导入wc\_in1表的数据文件data的内容。

    ```
     hello,odps
    ```

    导入wc\_in2表的数据文件data的内容。
...
在MaxCompute客户端中执行MultipleInOut。
...
作业成功结束后，mr\_multiinout\_out1中的内容如下。
